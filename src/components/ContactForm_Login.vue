  ...
  <div class="login-form">
    ...
    <form @submit.prevent="submitLogin">
      ...
      <router-link :to="{ name: 'contact.add' }">
        <button type="" class="btn btn-primary" style="margin-left: 24px;">Đăng kí</button>
        </router-link>
...
export default {
  ...
  methods: {
    async submitLogin() {
      try {
        const res = await contactService.login(this.loginData.username, this.loginData.password);
        if (res?.user?._id) {
